...
namespace Shiftnet.Apps
{
    public class GraphicPicker : Control, IPlexWindow
    {
                ...
                FileSkimmerBackend.GetFile(new[] { ".png", ".bmp", ".jpg", ".jpeg" }, FileOpenerStyle.Open, (file) =>
                 {
                     ...
                     {
                         Plex.Engine.Infobox.Show("Invalid image format", "The file you specified is not a valid picture format.");
                     }
                 });
            };
            _tile.Click += () =>
        ...
        public void LoadPreview()
        {
            ...
            if (_image == null)
                return;
            _preview.Image = _image.ToTexture2D(UIManager.GraphicsDevice);
            Invalidate();
        }
