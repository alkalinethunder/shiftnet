...
namespace Shiftnet.Apps
{
    ...
    public class MultiplayerServerList : Control, IPlexWindow
    {
        ...
        public MultiplayerServerList(Action _cb = null)
        {
            ...
            _add.Click += () =>
            {
                Plex.Engine.Infobox.PromptText("Name the server", "Please enter a name for this server so we can show it in the list.", (name) =>
                {
                    if(string.IsNullOrWhiteSpace(name))
                    {
                        Plex.Engine.Infobox.Show("Invalid name", "You can't put a blank name. Server not added.");
                        return;
                    }
                    var server = new ServerDetails();
                    server.FriendlyName = name;
                    Plex.Engine.Infobox.PromptText("Please enter the server's hostname", "Please enter the hostname we should connect to.\r\n\r\nExamples:\r\ntheplexnet.com\r\nlocalhost\r\n127.0.0.1\r\ntheplexnet.com:420\r\nlocalhost:1337", (hn) =>
                    {
                        int port = 3251;
                        string host = hn;
                        ...
                        {
                            Plex.Engine.Infobox.Show("Empty hostname.", "You cannot supply an empty hostname! Server not added.");
                            return;
                        }
                        if (hn.Contains(":"))
                        {
                        ...
                        if (parsePort)
                        {
                            ...
                            {
                                Plex.Engine.Infobox.Show("Invalid port.", "The port you entered (" + split[1] + ") is not a valid number.");
                                return;
                            }
                            if(port < 0 || port > 65535)
                            {
                                Plex.Engine.Infobox.Show("Invalid port.", "The port you entered (" + split[1] + ") is either too large or too small. Ports must be greater than or equal to 0, and less than or equal to 65535.");
                                return;
                            }
        ...
        public void ConnectToServer()
        {
            ...
            {
                Plex.Engine.Infobox.Show("Connection error.", ex.Message);
                return;
            }
            _callback?.Invoke();
            AppearanceManager.Close(this);
