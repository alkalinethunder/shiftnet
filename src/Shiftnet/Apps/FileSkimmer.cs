...
namespace Shiftnet.Apps
{
    ...
    public class FileSkimmer : Control, IPlexWindow
    {
        ...
        public FileSkimmer()
        {
            ...
            {
                Plex.Engine.Infobox.PromptText("New folder", "Please name your new folder.", (dname) =>
                {
                    if(string.IsNullOrWhiteSpace(dname))
                    {
                        Plex.Engine.Infobox.Show("New folder", "You can't have a blank directory name!");
                        return;
                    }
                    if(dname.Contains("/") || dname.Contains(":") || dname == "." || dname.Contains(".."))
                    {
                        Plex.Engine.Infobox.Show("New folder", $"Illegal folder name {dname}. Folders can't have '/' or ':' in their names and can't be either '.' or '..'.");
                        return;
                    }
                    string fpath = _currentdirectory + "/" + dname;
                    if (FSUtils.DirectoryExists(fpath))
                    {
                        Plex.Engine.Infobox.Show("New folder", "That folder already exists. Try specifying a different name?");
                        return;
                    }
                    try
                    {
                    ...
                    {
                        Plex.Engine.Infobox.Show("IO error", ex.Message);
                        return;
                    }
                    ResetList();
                });
                ...
                if(_fList.SelectedItem != null)
                {
                    try
                    {
                        ...
                        if (FileExists(path) || DirectoryExists(path))
                            Plex.Engine.Infobox.PromptYesNo("Delete file", "Are you sure you want to delete the file/directory at " + path + "?", (answer) =>
                            {
                                if(answer == true)
                                {
                                    Delete(path);
                ...
                if (_fList.SelectedItem != null)
                {
                    try
                    {
                        ...
                        if (FileExists(path))
                        {
                            ...
                            if(UIManager.FourthWall.GetFilePath($"Save {path} from Project: Shiftnet to disk", "Source file type|*"+ext, FileOpenerStyle.Save, out outputPath) == true)
                            {
            ...
            _importFile.ItemActivated += () =>
            {
                ...
                {
                    Plex.Engine.Infobox.Show("Import file", "You can't import files right now because there are no known file types to import.");
                    return;
                }

                if(UIManager.FourthWall.GetFilePath("Import file into Project: Shiftnet at " + _currentdirectory, filter, FileOpenerStyle.Open, out path))
                {
                    ...
                    if (FileExists(fpath))
                    {
                        Plex.Engine.Infobox.Show("File exists", "This file already exists.");
                        return;
                    }
        ...
        public void HandleFileSelect(bool quiet = false)
        {
            ...
            if (exists)
            {
                ...
                {
                    Plex.Engine.Infobox.PromptYesNo("Overwrite file?", "The file you chose already exists on disk. Would you like to overwrite it?", (answer) =>
                    {
                        if (answer)
                        {
                            DialogCallback?.Invoke(path);
            ...
            else
            {
                ...
                {
                    if (quiet == false)
                        Plex.Engine.Infobox.Show("File not found.", "The requested file path could not be found on disk.");
                }
                else
                {
                    DialogCallback?.Invoke(path);
        ...
        public void Navigate(string path)
        {
            ...
            else if (FileExists(path))
            {
                ...
                else
                {
                    ...
                    {
                        Plex.Engine.Infobox.Show("File Skimmer can't open this file!", "File Skimmer couldn't find a program that can open a file of this type. Please install a program that can handle this file and try again.");
                    }
                }
            }
        }
