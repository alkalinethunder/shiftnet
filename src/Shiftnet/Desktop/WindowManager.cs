...
namespace Shiftnet.Desktop
{
    ...
    public class WindowManager : Plex.Engine.WindowManager
    {
            ...
            if (brdr != null)
            {
                if (brdr.Close())
                {
                    ...
                    if (AppearanceManager.OpenForms.Contains(brdr))
                    {
                        ...
                        Plex.Engine.Desktop.ResetPanelButtons();
                    }
                    win = null;
                }
            }
        ...
        public bool DisplayObsolescenceIfAny(IPlexWindow win)
        {
            if (!(win is InfoboxMessage))
            {
                ...
                if(attrib != null)
                {
                    Plex.Engine.Infobox.Show($"{win.GetType().Name} is obsolete.", attrib.Message);
                    return true;
                }
            }
            return false;
    ...
    {
        private Plex.Engine.Theming.ButtonState _closeState = Plex.Engine.Theming.ButtonState.Idle;
        private Plex.Engine.Theming.ButtonState _minState = Plex.Engine.Theming.ButtonState.Idle;
        private Plex.Engine.Theming.ButtonState _maxState = Plex.Engine.Theming.ButtonState.Idle;
        ...
        {
            var _cstate = Plex.Engine.Theming.ButtonState.Idle;
            var _mxstate = Plex.Engine.Theming.ButtonState.Idle;
            var _mnstate = Plex.Engine.Theming.ButtonState.Idle;

            var _crect = ThemeManager.Theme.GetTitleButtonRectangle(TitleButton.Close, Width, Height);
            if(MouseX >= _crect.X && MouseX <= _crect.X + _crect.Width && MouseY >= _crect.Y && MouseY <= _crect.Y + _crect.Height)
            {
                _cstate = Plex.Engine.Theming.ButtonState.MouseHover;
                if (MouseLeftDown)
                    _cstate = Plex.Engine.Theming.ButtonState.MouseDown;
            }
            if (!IsDialog)
            {
                var _mxrect = ThemeManager.Theme.GetTitleButtonRectangle(TitleButton.Maximize, Width, Height);
                ...
                {
                    _mxstate = Plex.Engine.Theming.ButtonState.MouseHover;
                    if (MouseLeftDown)
                        _mxstate = Plex.Engine.Theming.ButtonState.MouseDown;
                }
                var _mnrect = ThemeManager.Theme.GetTitleButtonRectangle(TitleButton.Minimize, Width, Height);
                if (MouseX >= _mnrect.X && MouseX <= _mnrect.X + _mnrect.Width && MouseY >= _mnrect.Y && MouseY <= _mnrect.Y + _mxrect.Height)
                {
                    _mnstate = Plex.Engine.Theming.ButtonState.MouseHover;
                    if (MouseLeftDown)
                        _mnstate = Plex.Engine.Theming.ButtonState.MouseDown;
                }
